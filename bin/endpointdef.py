...
def listEvents(domainName,streamId,accountSwitchKey=None):
    ...
    listEventsEndpoint = '/config-media-live/v1/live/{domain}/stream/{streamId}/event'.format(domain=domainName,streamId=streamId)
    if accountSwitchKey:
        params = {'accountSwitchKey':accountSwitchKey}
...
def GetStream(domainName,streamId,accountSwitchKey=None):
    """ Get the Stream Info  """
    getstreamEndpoint = '/config-media-live/v1/live/{domain}/stream/{streamId}'.format(domain=domainName,streamId=streamId)
    if accountSwitchKey:
        params = {'accountSwitchKey':accountSwitchKey}
...
def GetRTMPStream(streamId,accountSwitchKey=None):
    ...
    getRTMPStreamEndpoint = '/config-media-live/v1/live/rtmp/stream/{streamId}'.format(streamId=streamId)
    if accountSwitchKey:
        params = {'accountSwitchKey':accountSwitchKey}
